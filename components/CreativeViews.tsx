...
export const ResearchView: React.FC<{ 
    ...
    const toggleTask = (id: string) => {
        const updated = (project.agentTasks || []).map(t => t.id === id ? { ...t, isCompleted: !t.isCompleted } : t);
        updateProject({ agentTasks: updated });
    };
                ...
                <div className="w-1/3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-6 shadow-sm overflow-y-auto hidden lg:block">
                    ...
                    {project.marketAnalysis ? (
                        <div className="space-y-6 text-sm">
                            <div><label className="text-xs text-gray-400 font-bold uppercase block mb-1">Title</label><p className="font-serif text-lg font-bold text-gray-900 dark:text-gray-100">{project.marketAnalysis.suggestedTitle}</p></div>
                            
                            <div><label className="text-xs text-gray-400 font-bold uppercase block mb-1">Logline</label><p className="text-gray-600 dark:text-gray-300 leading-relaxed">{project.marketAnalysis.narrativeStructure}</p></div>
                            
                            <div className="grid grid-cols-2 gap-4">
                                <div><label className="text-xs text-gray-400 font-bold uppercase block mb-1">Visual Style</label><span className="bg-pink-50 dark:bg-pink-900/30 text-pink-700 dark:text-pink-300 px-2 py-1 rounded border border-pink-100 dark:border-pink-800 font-medium inline-block text-xs">{project.marketAnalysis.visualStyle}</span></div>
                                <div><label className="text-xs text-gray-400 font-bold uppercase block mb-1">Audience</label><p className="text-gray-600 dark:text-gray-300 text-xs font-medium">{project.marketAnalysis.targetAudience}</p></div>
                            </div>
                            ...
                            <div className="pt-4 border-t border-gray-100 dark:border-gray-700">
                                ...
                                <p className="text-gray-800 dark:text-gray-200 font-medium mb-2">{project.marketAnalysis.worldSetting}</p>
                            ...
                            {project.marketAnalysis.keyThemes && project.marketAnalysis.keyThemes.length > 0 && (
                                <div>
                                    ...
                                    <div className="flex flex-wrap gap-1">
                                        {project.marketAnalysis.keyThemes.map(theme => (
                                            <span key={theme} className="text-[10px] bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 px-2 py-1 rounded-full border border-gray-200 dark:border-gray-600">#{theme}</span>
                                        ))}
                                    </div>
                                </div>
                            )}
            ...
            <div className="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-6 overflow-hidden">
                 <div className="lg:col-span-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-sm flex flex-col overflow-hidden">
                      <div className="p-4 border-b border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 flex items-center justify-between">
                          ...
                          {project.panels.length > 0 && <span className="text-xs bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300 px-2 py-0.5 rounded-full font-bold">{project.panels.length} {t('ui.panels')}</span>}
                      </div>
                 ...
                 <div className="lg:col-span-1 space-y-4 overflow-y-auto">
                      ...
                      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-4 shadow-sm">
                           ...
                           <div className="space-y-2">
                               ...
                               {project.characters.length === 0 && <span className="text-xs text-gray-400 italic">{t('err.no_chars')}</span>}
                           </div>
                      </div>
                 </div>
            </div>
...
}> = ({ project, handleFinishCharacterDesign, handleRegenerateSingleCharacter, handleUpdateCharacterDescription, handleUpdateCharacterVoice, toggleCharacterLock, handleCharacterUpload, handleCheckConsistency, handleSelectCharacterVariant, role, t, availableVoices }) => {
    
    // Local state to manage style selection per character card to avoid global state clutter
    ...
    return (
        <div className="max-w-7xl mx-auto w-full px-6 pb-24">
            <div className="flex items-center justify-between mb-8">
                ...
                <button onClick={handleFinishCharacterDesign} className="bg-purple-600 hover:bg-purple-700 text-white px-6 py-3 rounded-xl font-bold flex items-center gap-2 shadow-lg shadow-purple-200 dark:shadow-none transition-all">
                    ...
                    <div key={char.id} className={`bg-white dark:bg-gray-800 border ${char.consistencyStatus === 'FAIL' ? 'border-red-400 dark:border-red-500 ring-2 ring-red-100 dark:ring-red-900/20' : 'border-gray-200 dark:border-gray-700'} rounded-2xl overflow-hidden shadow-sm hover:shadow-lg transition-all group flex flex-col h-full`}>
                         <div className="aspect-square bg-gray-50 dark:bg-gray-900 relative overflow-hidden flex items-center justify-center shrink-0">
                             ...
                             <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3 backdrop-blur-sm">
                                  ...
                                  <button onClick={() => handleCheckConsistency(char, idx)} className="p-3 bg-white rounded-full text-gray-800 hover:text-blue-600 shadow-lg transform hover:scale-110 transition-all" title="Check Consistency">
                         ...
                         <div className="p-5 flex-1 flex flex-col">
                             ...
                             <div className="space-y-4 flex-1">
                                ...
                                <div className="pt-2 border-t border-gray-100 dark:border-gray-700">
                                    <label className="text-[10px] font-bold text-gray-400 uppercase block mb-2">Style Generator</label>
                                    <div className="flex gap-2">
                                        <select 
                                        ...
                                        <button 
                                            ...
                                            className="bg-purple-100 dark:bg-purple-900/50 text-purple-700 dark:text-purple-300 p-2 rounded-lg hover:bg-purple-200 dark:hover:bg-purple-800 transition-colors shadow-sm"
...
export const PanelArtistView: React.FC<{
    ...
    const hasStartedGeneration = project.panels.some(p => p.imageUrl || p.isGenerating);
        ...
        <div className="p-8 max-w-6xl mx-auto pb-24">
            ...
            {!hasStartedGeneration ? (
                <div className="bg-gray-50 dark:bg-gray-800 border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-2xl h-64 flex flex-col items-center justify-center text-gray-400 dark:text-gray-500 gap-4">
                    ...
                    <p className="font-medium">Ready to visualize {project.panels.length} panels.</p>
